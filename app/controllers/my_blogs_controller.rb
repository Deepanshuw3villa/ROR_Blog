class MyBlogsController < ApplicationController
  ...
  if !cate.nil?
    @my_blogs = MyBlog.where(:category_id => cate)
  else
    @my_blogs = MyBlog.all
  end
end
  ...
  def create
    ...
    respond_to do |format|
      if @my_blog.save
        format.html { redirect_to my_blog_url(@my_blog), notice: "My blog was successfully created." }
  ...
  def update
    respond_to do |format|
      if @my_blog.update(my_blog_params)
        format.html { redirect_to my_blog_url(@my_blog), notice: "My blog was successfully updated." }
  ...
  private
    ...
    def my_blog_params
      params.require(:my_blog).permit(:title, :body, :category_id, :image, :user_id)
    end
end
